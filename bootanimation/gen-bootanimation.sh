...
if [ $PARAM_TARGET_SCREEN_HEIGHT -lt $PARAM_TARGET_SCREEN_WIDTH ]; then
    BASE_WIDTH=$PARAM_TARGET_SCREEN_HEIGHT
else
    BASE_WIDTH=$PARAM_TARGET_SCREEN_WIDTH
fi

if [ "$PARAM_TARGET_BOOTANIMATION_HALF_RES" = "true" ]; then
    IMAGEWIDTH=$(echo "$BASE_WIDTH * 0.75" | bc)
else
    IMAGEWIDTH=$BASE_WIDTH
fi

IMAGEHEIGHT=$(echo "$IMAGEWIDTH * 10 / 16" | bc)
RESOLUTION="$IMAGEWIDTH"x"$IMAGEHEIGHT"

$PARAM_MOGRIFY -resize $RESOLUTION -colors 256 $INTERMEDIATES/*/*.png;

echo "$IMAGEWIDTH $IMAGEHEIGHT 25" > $INTERMEDIATES/desc.txt;
